<template>
	<table class="table table-striped mb-0">
		<tbody>
			...
			<tr>
				<td class="font-weight-600">Total {{ invoiceFieldName | capitalize }}</td>
				<td class="font-weight-600 text-primary">$ {{ invoiceTotalPrice | floating }}</td>
			</tr>
			<tr>
				<td class="font-weight-400">Paid</td>
				<td :class="`font-weight-400 text-${paidVariant}`">$ {{ invoice.paid | floating }}</td>
			</tr>
			<tr>
				<td class="font-weight-400">Due</td>
				<td :class="`font-weight-400 text-${dueVariant}`">$ {{ invoice.due | floating }}</td>
			</tr>
		</tbody>
	</table>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			dueVariant() {
				return this.invoice.due > 0 ? "danger" : "success";
			},

			paidVariant() {
				let paid = this.invoice.paid,
					due = this.invoice.due,
					total = this.invoiceTotalPrice;

				if (paid == total) return "success";

				if (paid > due && paid < total) return "orange"; // partial
